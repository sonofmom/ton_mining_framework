...
def render_tvm_element(element_type, element):
    if element_type in ["num", "number", "int"]:
        ...
        return {'@type': 'tvm.stackEntryCell',
                'cell': {'@type': 'tvm.Cell', 'bytes': element.serialize_boc(has_idx=False)}}
    elif element_type == "slice":
        element = deserialize_cell_from_json(element)
        return {'@type': 'tvm.stackEntrySlice',
                'slice': {'@type': 'tvm.Slice', 'bytes': element.serialize_boc(has_idx=False)}}
    else:
