...
# define HEAD_SIZE_WORDS 11
# define HEAD_SIZE_BYTES 43
# define RAND_SIZE_WORDS 7
# define RAND_SIZE_BYTES 28
# define SEED_SIZE_WORDS 4
# define SEED_SIZE_BYTES 16
# define WORK_SIZE_WORDS 13
# define WORK_SIZE_BYTES 52

KERNEL_FQ void crunch(
        __global const u32x *data_head,
        __global const u32x *data_rand,
        __global u32x *data_counter,
    ...
    */
    ...
    printf("\n");
    printf("    Data rand     : ");
    printf("%08x", data_rand[(RAND_SIZE_WORDS*gid)+0]);
    printf("%08x", data_rand[(RAND_SIZE_WORDS*gid)+1]);
    printf("%08x", data_rand[(RAND_SIZE_WORDS*gid)+2]);
    printf("%08x", data_rand[(RAND_SIZE_WORDS*gid)+3]);
    printf("%08x", data_rand[(RAND_SIZE_WORDS*gid)+4]);
    printf("%08x", data_rand[(RAND_SIZE_WORDS*gid)+5]);
    printf("%08x", data_rand[(RAND_SIZE_WORDS*gid)+6]);
    printf("\n");
    printf("    Data counter  : ");
    printf("%08x", data_counter[gid]);
    printf("\n");
    printf("    Data seed     : ");
    ...
    */
    ...
    if ((data_counter[gid] + iterations_max) > u32size) {
        data_counter[gid] -= iterations_max;
    }
    ...
    */
    u32x p[64] = { 0 };
    p[0] = data_rand[(RAND_SIZE_WORDS*gid)+0];
    p[1] = data_rand[(RAND_SIZE_WORDS*gid)+1];
    p[2] = data_rand[(RAND_SIZE_WORDS*gid)+2];
    p[3] = data_rand[(RAND_SIZE_WORDS*gid)+3];
    p[4] = data_rand[(RAND_SIZE_WORDS*gid)+4];
    p[5] = data_rand[(RAND_SIZE_WORDS*gid)+5];
    p[6] = data_rand[(RAND_SIZE_WORDS*gid)+6];

    u32x w[64] = { 0 };
    w[0] = data_counter[gid];
    w[1] = data_seed[0];
    w[2] = data_seed[1];
    w[3] = data_seed[2];
    w[4] = data_seed[3];
    w[5] = data_rand[(RAND_SIZE_WORDS*gid)+0];
    w[6] = data_rand[(RAND_SIZE_WORDS*gid)+1];
    w[7] = data_rand[(RAND_SIZE_WORDS*gid)+2];
    w[8] = data_rand[(RAND_SIZE_WORDS*gid)+3];
    w[9] = data_rand[(RAND_SIZE_WORDS*gid)+4];
    w[10] = data_rand[(RAND_SIZE_WORDS*gid)+5];
    w[11] = data_rand[(RAND_SIZE_WORDS*gid)+6];
    w[12] = data_counter[gid];

    sha256_ctx_t shactx1;
    sha256_ctx_t shactx2;
    sha256_init (&shactx1);
    sha256_update_global (&shactx1, data_head, HEAD_SIZE_BYTES);
    sha256_update (&shactx1, p, RAND_SIZE_BYTES);
    ...
    for (iterations_counter = 0; iterations_counter < iterations_max; iterations_counter++) {
        ...
        if (
            ...
            */
            ...
            for (int t = 0; t < RAND_SIZE_WORDS; t++) {
                solution[HEAD_SIZE_WORDS+t] = p[t];
            }
            for (int t = 0; t < WORK_SIZE_WORDS; t++) {
                solution[(HEAD_SIZE_WORDS+RAND_SIZE_WORDS)+t] = w[t];
            }
            break;
        }
